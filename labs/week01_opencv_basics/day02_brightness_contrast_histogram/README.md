...
2. 이미지가 없어도 실행한다. 이 경우 스크립트가 학습용 synthetic 이미지를 자동으로 만든다.
...
- [ ] `data/raw/`에 직접 찍은 이미지를 넣어본다.
- [ ] 원본 이미지가 어두운지, 밝은지, 반사가 있는지 관찰한다.
- [ ] 밝기 조정 결과에서 255 포화 영역이 생기는지 확인한다.
- [ ] 대비 조정 결과에서 경계가 좋아지는지 확인한다.
- [ ] 대비 조정 후 노이즈나 반사가 더 튀는지 확인한다.
- [ ] 히스토그램이 왼쪽/오른쪽/좁은 구간 중 어디에 몰리는지 기록한다.
- [ ] 히스토그램 평활화가 도움이 되는 경우와 실패하는 경우를 기록한다.
- [ ] `notes/day02_log.md`에 오늘 관찰 내용을 직접 적는다.
